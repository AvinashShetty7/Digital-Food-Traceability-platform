...
export default function Viewmanudocs() {
  const { manufacturerId } = useParams(); 
  
  const API_URL = import.meta.env.VITE_API_URL;
  const navigate = useNavigate();
  const [manufacturer, setFarmer] = useState(null);
    ...
    const fetchFarmer = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_URL}/api/admin/singlemanufacturer/${manufacturerId}`,
          { withCredentials: true }
        ); 
        console.log(res.data);
        
        setFarmer(res.data.manufacturers); // adjust based on your API response
      } catch (err) {
        console.error(err);
        setError("Failed to load manufacturer documents");
      } finally {
        setLoading(false);
      }
    };

    if (manufacturerId) fetchFarmer();
  }, [manufacturerId]);

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <p className="text-gray-600 text-lg">Loading manufacturer documents...</p>
      </div>
    );
  }
  ...
  console.log(manufacturer.documents);
  
  const docsToShow = manufacturer.documents?.slice(0, 5) || [];
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-600">
              Farmer ID: <span className="font-mono">{manufacturerId}</span>
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm text-slate-700">
            <p><span className="font-medium">Email:</span> {manufacturer.email}</p>
            <p><span className="font-medium">Phone:</span> {manufacturer.phone}</p>
            <p><span className="font-medium">Role:</span> {manufacturer.role}</p>
          </div>
        </div>
